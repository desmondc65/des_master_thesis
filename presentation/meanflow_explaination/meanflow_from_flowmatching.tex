...
Regress the network onto its own identity. With the bootstrap target
\begin{equation}
  u_{\mathrm{tgt}} \;=\; v + (t-r)\big(v\,\partial_z u_\theta + \partial_r u_\theta\big),
  \label{eq:mf_target}
\end{equation}
the loss is the thesis MeanFlow objective (Chapter~3, Eq.~(3.13)),
\begin{equation}
  \mathcal{L}_{\mathrm{MF}}
   \;=\; \Exp\Big[\,\sg(w)\,
        \bnormb{\,u_\theta(z_r,r,t,\ct) - \sg(u_{\mathrm{tgt}})\,}^2\,\Big]
   \;+\; \lambda_{\mathrm{spec}}\,\mathcal{L}_{\mathrm{spec}}.
  \label{eq:mf_loss}
\end{equation}
Here $\sg[\cdot]$ is stop-gradient --- the whole target is a constant --- the
adaptive weight
\begin{equation}
  w \;=\; \big(\lVert\Delta\rVert_{2,\beta}^2 + \varepsilon_w\big)^{-p},
  \qquad p=1,\;\;\varepsilon_w=10^{-3},
  \label{eq:mf_weight}
\end{equation}
down-weights high-error samples ($p=0$ recovers plain weighted MSE), and
$\lambda_{\mathrm{spec}}\mathcal{L}_{\mathrm{spec}}$ is the \code{qpepre} log-PSD
term inherited from FlowCast (active on the $r=t$ subset). The derivative inside
$u_{\mathrm{tgt}}$ is one forward-mode \code{torch.func.jvp} with tangent
$(v,1,0)$, under \code{no\_grad} on the bare module. Two flow times are drawn per
sample and sorted to $r\le t$; a fraction $\rho_{\mathrm{MF}}=0.25$ keeps $r<t$
(the bootstrapped, finite-interval target), and the other $75\%$ collapse to
$r=t$, where \eqref{eq:mf_boundary} makes the loss \emph{exactly} the FlowCast
I-CFM regression --- the clean, data-grounded anchor the off-diagonal samples
propagate outward.
