...
\end{align}
Each step uses two denoiser evaluations except the final step where the correction is skipped. With $N = 18$ steps this yields roughly $2N-1 = 35$ NFE; StormCast uses a shorter $N \approx 13$ schedule giving $\approx 25$ NFE. This $\approx 25$-NFE Heun sampler is the cost figure that the straight-line FlowCast sampler of Chapter~\ref{ch:method} is benchmarked against: every NFE that FlowCast saves at matched forecast quality is a direct operational speedup.

% ==================================================
\section{Conditioning in Diffusion Models}
\label{sec:bg-conditioning}

Conditional diffusion models can be implemented either by concatenating the condition $c$ along the channel axis of the denoiser's input, or by classifier-free guidance (CFG) \citep{ho2022cfg}, which trains the network jointly on conditional and unconditional tasks and linearly combines their outputs at inference time. StormCast uses the simpler concatenation approach: the conditioning bundle $c = \mathrm{concat}(X_{t-1}, S_t, M_t, I)$ is stacked with the noised target along the channel dimension and fed to the SongUNet. No classifier-free guidance is used. FlowCast inherits the same channel-concatenation \emph{mechanism} for its velocity field, though it conditions on a slightly trimmed bundle that omits the low-resolution background $S_t$ (Section~\ref{sec:method-flowcast}); the fundamental change between the diffusion baseline and FlowCast is the generative process itself, not the way conditioning enters the network.

% ==================================================
\section{Flow Matching and FlowCast}
\label{sec:bg-flowmatching}

Conditional Flow Matching (CFM) is a recently-proposed alternative to score-based diffusion that learns the \emph{same} kind of noise-to-data generative model --- a deterministic flow integrated from a simple prior to the data distribution --- but with a training objective that bypasses score matching entirely. CFM is the generative method at the centre of this thesis (the FlowCast residual of Section~\ref{sec:method-flowcast}), so we give it the same theoretical treatment as DDPM, DDIM, and EDM.
